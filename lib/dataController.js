...
class DataController {
            ...
            for (const [stateName, val] of Object.entries(inverter)) {
                switch (stateName) {
                    ...
                    case 'DC':
                        ...
                        break;
                    case 'INV':
                        await this.processINVData(inverter.serial, val);
                        break;
                    default:
    ...
    async processINVData(serial, val) {
        // Create channel
        if (!createCache.includes(`${serial}.inv`)) {
            const deviceObj = {
                type: 'channel',
                common: {
                    name: 'INV',
            ...
            // @ts-ignore
            await this.adapter.extendObjectAsync(`${serial}.inv`, deviceObj);
            createCache.push(`${serial}.inv`);
        }
        ...
        // Inverter x
        for (const [inverter, inverterObj] of Object.entries(val)) {
            // Create channel
            const inverterNumber = Number(inverter) + 1;
            if (!createCache.includes(`${serial}.inv.inverter_${inverterNumber}`)) {
                channelObj.common = {
                    name: `Inverter ${inverterNumber}`,
                };
                // @ts-ignore
                await this.adapter.extendObjectAsync(`${serial}.inv.inverter_${inverterNumber}`, channelObj);
                createCache.push(`${serial}.inv.inverter_${inverterNumber}`);
            }

            // Create and/or set values
            for (const [inverterStateName, inverterVal] of Object.entries(inverterObj)) {
                this.setObjectAndState(serial, `inv_${inverterStateName.toLowerCase()}`, inverterVal, inverterNumber);
            }
        }
    }
    ...
    async setObjectAndState(stateID, stateName, val, count) {
        ...
        if (val !== undefined) {
            ...
            // yieldtotal Protection against incorrect zeroing when the OpenDTU restarts if the inverter is not accessible.
            if (fullStateID.endsWith('yieldtotal')) {
                if (Number(value) <= 0) {
                    return;
                }
            }
